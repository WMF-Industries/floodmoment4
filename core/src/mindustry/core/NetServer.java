...
public class NetServer implements ApplicationListener{
    ...
    public TeamAssigner assigner = (player, players) -> {
        if(state.rules.pvp){
            ...
            TeamData re = state.teams.getActive().min(data -> {
                if((state.rules.waveTeam == data.team && state.rules.waves) || !data.team.active() || data.team == Team.derelict || data.team == creeperTeam) return Integer.MAX_VALUE;
    ...
    private void registerCommands(){
        clientCommands.<Player>register("tutorial", "", "Start the interactive tutorial", (args, player) -> { // TODO: Why are we calling menuChoose instead of menu?
