    ...
    DynamicTest[] testSectorValidity(){
        ...
        for(SectorPreset zone : content.sectors()){
            ...
            out.add(dynamicTest(zone.name, () -> {
                ...
                assertTrue(spawner.countSpawns() > 0 || (state.rules.attackMode && state.teams.get(state.rules.waveTeam == Team.blue ? Team.crux : state.rules.waveTeam).hasCore()), "Sector \"" + zone.name + "\" has no enemy spawn points: " + spawner.countSpawns());
            }));
        }
